...
def add_extension():
    ...
    if not cat: return
    ext = simpledialog.askstring("New Extension", f"Enter extension for {cat} (with dot):")
    if not ext or not ext.startswith("."): return
    if ext in TYPES[cat]:
        messagebox.showinfo("Info", "Extension already exists!")
        return
    TYPES[cat].append(ext)
    update_category_list()
...
def undo_move():
    if not undo_history:
        ...
        return
    for orig, dest in undo_history.pop().items():
...
tk.Label(frame, text="Categories & Extensions:",
         bg="#111", fg="#0ff").pack()

category_listbox = tk.Listbox(frame, width=50, height=8)
...
sort_btn = tk.Label(root, text="Select Folder & Organize",
                    font=("Arial", 14, "bold"), bg="#27AE60", fg="white", relief="flat", padx=20, pady=10)
sort_btn.bind("<Button-1>", organize_files)
sort_btn.pack(pady=10)

tk.Label(root, text="Recursive sorting: Images, Docs, Videos, Audio, Code + Custom",
         bg="#111", fg="#555", font=("Arial", 10)).pack(side="bottom", pady=10)
